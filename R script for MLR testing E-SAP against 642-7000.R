...
multiLR <- read.delim("C:/Users/Robert/Google Drive/Mullet Lab/Projects/QTL Mapping projects/BTx642 x Tx7000/642-7000-ESAP-compxconver_R-input.txt")
multiLR <- multiLR[complete.cases(multiLR),]
View(multiLR)
GlucFIT = standardize(lm(yield.glucose ~ stem.length +  int.diameter +  wall.density + percent.wall +  percent.lignin.in.wall + grams.cellulose, data=multiLR))
sum.GlucFIT = summary(GlucFIT)
sum.GlucFIT
PentFIT = standardize(lm(yield.pentose ~ stem.length +  int.diameter +  wall.density +  + percent.wall + percent.lignin.in.wall + grams.xylan, data=multiLR))
...
GlucFIT.vif <- vif(standardize(lm(yield.glucose ~ stem.length +  int.diameter +  wall.density + percent.wall +  percent.lignin.in.wall + grams.cellulose, data=multiLR)))
GlucFIT.vif
PentFIT.vif <- vif(standardize(lm(yield.pentose ~ stem.length +  int.diameter +  wall.density +	percent.wall + percent.lignin.in.wall + grams.xylan, data=multiLR)))
PentFIT.vif
...
GlucFIT2 = lm(yield.glucose ~ stem.length +  int.diameter +  wall.density + percent.wall + percent.lignin.in.wall + grams.cellulose, data=multiLR)
PentFIT2 = lm(yield.pentose ~ stem.length +  int.diameter +  wall.density + percent.wall + percent.lignin.in.wall + grams.xylan, data=multiLR)
